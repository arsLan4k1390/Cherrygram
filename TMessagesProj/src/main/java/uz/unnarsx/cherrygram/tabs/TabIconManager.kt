...
object TabIconManager {
    ...
    private val file = File(ApplicationLoader.applicationContext.getDir("cg_goodies", Context.MODE_PRIVATE), "CherrygramFolderCache.cft")
    ...
    fun injectTabTitle(tab: FilterTabsView.Tab): String {
        ...
        return when {
            CherrygramConfig.newTabs_iconsV2_replace -> icon
            CherrygramConfig.newTabs_iconsV2_append -> "$icon ${tab.title}"
            else -> tab.title
        }
    }*/
    ...
    fun injectTabTitle(id: Int, fallback: String): String {
        ...
        return when (CherrygramConfig.newTabs_iconsV2_mode) {
            1 -> "$icon $fallback"
            2 -> icon
            else -> fallback
        }
    ...
    fun getIconForTab(id: Int): String {
        return if (mappedEmojis.containsKey(id) && mappedEmojis[id] != null) mappedEmojis[id]!! else CherrygramExtras.wrapEmoticon(null)
    }
