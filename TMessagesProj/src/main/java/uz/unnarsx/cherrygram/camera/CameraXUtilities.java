...
public class CameraXUtilities {
    ...
    private static void loadSuggestedResolution() {
        ...
        getAvailableVideoSizes().values().stream()
                ...
                .ifPresent(height -> {
                    ...
                    if (CherrygramConfig.INSTANCE.getCameraResolution() == -1 || CherrygramConfig.INSTANCE.getCameraResolution() > maxRes || CherrygramConfig.INSTANCE.getCameraResolution() < lowRes) {
                        CherrygramConfig.INSTANCE.saveCameraResolution(height);
                    }
                });
    }
    ...
    public int getCurrentResolution() {
        return getAvailableVideoSizes().values().stream()
                .mapToInt(Size::getHeight)
                .filter(size -> size == CherrygramConfig.INSTANCE.getCameraResolution())
                .findFirst()
                .orElse(-1);
    }
