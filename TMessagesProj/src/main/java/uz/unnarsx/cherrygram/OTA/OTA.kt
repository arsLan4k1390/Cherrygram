...
object OTA : CoroutineScope by MainScope() {
    ...
    private fun checkBS(callback: (Boolean) -> Unit) {
        launch(handler) {
            try {
                val request: Request =
                        Request.Builder().url("https://api.github.com/repos/arsLan4k1390/Cherrygram/releases/latest").build()
                withContext(Dispatchers.IO) {
                    val response = OkHttpClient().newCall(request).execute()
                    parseddString = response.body!!.string()
                    val parsedString = JSONObject(parseddString)
                    if (parsedString.getString("name") > CherrygramExtras.CG_VERSION) {
    ...
    fun downloadApk(context: Context) {
        val notificationManager = NotificationManagerCompat.from(context)
        notificationManager.cancel(1337)
        val request = DownloadManager.Request(Uri.parse("https://github.com/arsLan4k1390/Cherrygram/releases/latest/download/Cherrygram.apk"))

        request.setAllowedNetworkTypes(DownloadManager.Request.NETWORK_MOBILE or DownloadManager.Request.NETWORK_WIFI)
        request.setTitle("Cherrygram v$version")
        request.setNotificationVisibility(DownloadManager.Request.VISIBILITY_VISIBLE_NOTIFY_COMPLETED)
        request.setDestinationInExternalPublicDir(Environment.DIRECTORY_DOWNLOADS, "Cherrygram v$version.apk")
