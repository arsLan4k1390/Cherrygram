        ...
        listView.setOnItemClickListener((view, position, x, y) -> {
            ...
            if (position == signMessagesRow) {
                ...
                if (isExpandableSendMediaRow(position)) {
                    ...
                    return;
                } else if (position > permissionsSectionRow && position <= Math.max(manageTopicsRow, changeInfoRow) && view instanceof TextCheckCell2 || position == manageLinkedPeersRow) {
    ...
    public static void sortAdmins(ArrayList<TLObject> participants) {
        participants.sort((lhs, rhs) -> {
            int result = Integer.compare(
                    getChannelAdminParticipantType(lhs),
                    getChannelAdminParticipantType(rhs)
            );
            ...
            if (lhs instanceof TLRPC.ChannelParticipant l && rhs instanceof TLRPC.ChannelParticipant r) {
                return Long.compare(MessageObject.getPeerId(l.peer), MessageObject.getPeerId(r.peer));
            }
            return 0;
        });
    }
