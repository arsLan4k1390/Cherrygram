...
public class PasskeysActivity extends BaseFragment {
    ...
    private void fillItems(ArrayList<UItem> items, UniversalAdapter adapter) {
        ...
        items.add(UItem.asButton(-2, R.drawable.msg_settings, getString(R.string.Settings)).accent());
        ...
        items.add(UItem.asShadow(AndroidUtilities.replaceMultipleTags(getString(R.string.CG_PasskeyInfo2),
            () -> Browser.openUrl(getContext(), getBitwardenUrl()),
            () -> Browser.openUrl(getContext(), getKeepassdxUrl())
        )));
    }
                ...
                if (error != null) {
                    ...
                    if (error.startsWith("No create options")) {
                        BulletinFactory.of(this).createSimpleBulletin(
                                R.raw.error,
                                getString(R.string.CG_PasskeyUnsupportedTitle),
                                AndroidUtilities.replaceMultipleTags(getString(R.string.CG_PasskeyUnsupportedMessage),
                                        () -> Browser.openUrl(getContext(), getBitwardenUrl()),
                                        () -> Browser.openUrl(getContext(), getKeepassdxUrl())
                                )
                        ).show();
                ...
                if (error != null) {
                    if (error.startsWith("No create options")) {
                        BulletinFactory.of(sheet.topBulletinContainer, sheet.getResourcesProvider()).createSimpleBulletin(
                                ...
                                getString(R.string.CG_PasskeyUnsupportedTitle),
                                AndroidUtilities.replaceMultipleTags(getString(R.string.CG_PasskeyUnsupportedMessage),
                                        () -> Browser.openUrl(context, getBitwardenUrl()),
                                        () -> Browser.openUrl(context, getKeepassdxUrl())
                                )
                        ).show();
                    } else {
    ...
    private static String getBitwardenUrl() {
        if (ApplicationLoader.checkPlayServices()) {
            ...
            return "https://github.com/bitwarden/android";
        }
    }

    private static String getKeepassdxUrl() {
        if (ApplicationLoader.checkPlayServices()) {
            return "https://play.google.com/store/apps/details?id=com.kunzisoft.keepass.free";
        } else {
