...
public class ChatActivityTopPanelLayout extends AnimatedLinearLayout {
    ...
    private void checkBoundsAndClipping() {
        ...
        final float r = Math.min(dp(CherrygramChatsConfig.INSTANCE.getCenterChatTitle() ? 24 : 18), Math.min(clipRectF.width(), clipRectF.height()) / 2f);
        clipPath.rewind();
        ...
        if (backgroundDrawable != null) {
            ...
            backgroundDrawable.setRadius(Math.min(dp(CherrygramChatsConfig.INSTANCE.getCenterChatTitle() ? 24 : 18), bgHeight / 2));
        }
    }
