...
public class ButtonWithCounterView extends FrameLayout implements Loadable {
    ...
    public ButtonWithCounterView(Context context, boolean filled, boolean useStroke, Theme.ResourcesProvider resourcesProvider) {
        super(context);
...
//        info.setContentDescription(text.getText() + (lastCount > 0 ? ", " + LocaleController.formatPluralString("Chats", lastCount) : ""));
    ...
    public ButtonWithCounterView setNeutral(boolean md3) {
        this.neutral = true;
        setFilled(true);
        setColor(Theme.getColor(md3 ? Theme.key_windowBackgroundWhite : Theme.key_buttonNeutral, resourcesProvider));
        updateColors();
        return this;
    }
    /** Cherrygram finish */
