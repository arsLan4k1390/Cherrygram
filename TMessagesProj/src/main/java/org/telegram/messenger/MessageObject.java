...
import uz.unnarsx.cherrygram.donates.adsgram.AdsGramResponse;
...
public class MessageObject {
    ...
    public boolean hasMediaSpoilers() {
        return !isRepostPreview && (messageOwner.media != null && messageOwner.media.spoiler || needDrawBluredPreview()) || isHiddenSensitive() || hasMediaSpoilersCG(false);
    }
...
//        }
    ...
    public boolean addEntitiesToText(CharSequence text, boolean photoViewer, boolean useManualParse) {
        ...
        if (isRestrictedMessage || getMedia(messageOwner) instanceof TLRPC.TL_messageMediaUnsupported) {
            ...
            return addEntitiesToText(text, MessagesFilterHelper.INSTANCE.addSpoilerEntities(this, getEntities()), isOutOwner(), true, photoViewer, useManualParse);
        }
    }
    ...
    public boolean needDrawShareButton() {
        ...
        if (type == TYPE_JOINED_CHANNEL) {
            ...
            return true;
        } else if (messageOwner.noforwards || isVoiceOnce() || isRoundOnce()) {
            return false;
        ...
        if (searchType == ChatActivity.SEARCH_PUBLIC_POSTS || messageOwner.fwd_from != null && !isOutOwner() && messageOwner.fwd_from.saved_from_peer != null && getDialogId() == UserConfig.getInstance(currentAccount).getClientUserId()) {
            return true;
        }
    ...
    public int getMaxMessageTextWidth() {
        ...
        if (maxWidth == 0) {
            maxWidth = generatedWithMinSize - dp(type == TYPE_ARTICLE ? 40 : isWideModeSupported() ? getWideMessageBounds() : 80);
            if (sideMenuEnabled) {
                maxWidth -= dp(64);
            } else if (needDrawAvatarInternal() && !isOutOwner() && !messageOwner.isThreadMessage) {
                maxWidth -= dp(52);
...
//        textHeight = 0;
    ...
    public long getChatId() {
        if (messageOwner.peer_id instanceof TLRPC.TL_peerChat) {
            ...
            return messageOwner.peer_id.user_id;
        }
        return 0;
    }
...
//        if (cachedSavedTimestamp != null) return cachedSavedTimestamp;
    ...
    public AdsGramResponse.BannerData bannerData;
    ...
    public boolean hasMediaSpoilersCG(boolean notifications) {
        long chatID;
        boolean require;
        ...
        return shouldBlockMessage() && (CherrygramMessagesConfig.INSTANCE.getMsgFiltersHideAllUnderSpoiler() || CherrygramMessagesConfig.INSTANCE.getMsgFiltersHideFromBlocked()) || addSpoiler;
    }

    public boolean isWideModeSupported() {
        final TLRPC.Chat chat = MessagesController.getInstance(currentAccount).getChat(-getDialogId());
        return CherrygramMessagesConfig.INSTANCE.getWideMessagesLayout()
//                && chat != null
                && ChatObject.isChannelCG(chat)
                && !chat.signature_profiles
                && this.messageOwner.grouped_id == 0 // Albums
                && !this.isSponsored() && !this.isAdsGram; // ADS
    }

    public int getWideMessageBounds() {
        if (messageOwner != null && messageOwner.media != null && messageOwner.media.document != null) { // Files
            CharSequence caption = ChatsHelper.getInstance(currentAccount).getMessageContent(this);
            boolean hasCaption = caption != null && caption.length() > 0 /*messageOwner.message != null && !messageOwner.message.isEmpty()*/;
            if (hasCaption) {
                return 60;
            }
        }
