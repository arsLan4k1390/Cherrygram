...
public class MessagesStorage extends BaseController {
    ...
    public void openDatabase(int openTries) {
        ...
        try {
            ...
            if (createTable) {
                ...
                database.executeFast("CREATE TABLE dialog_filter_cherry(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT, emoticon TEXT)").stepThis().dispose();
    ...
    private void updateDbToLastVersion(int currentVersion) throws Exception {
        ...
        if (version == 64) {
            database.executeFast("CREATE TABLE IF NOT EXISTS dialog_filter_cherry(id INTEGER PRIMARY KEY, ord INTEGER, unread_count INTEGER, flags INTEGER, title TEXT)").stepThis().dispose();
    ...
    private void loadDialogFilters() {
        storageQueue.postRunnable(() -> {
            try {
                ...
                SQLiteCursor filtersCursor = database.queryFinalized("SELECT id, ord, unread_count, flags, title, emoticon FROM dialog_filter_cherry WHERE 1");
                ...
                if (!hasDefaultFilter) {
                    ...
                    SQLitePreparedStatement state = database.executeFast("REPLACE INTO dialog_filter_cherry VALUES(?, ?, ?, ?, ?, ?)");
    ...
    public void saveDialogFiltersOrderInternal() {
        try {
            SQLitePreparedStatement state = database.executeFast("UPDATE dialog_filter_cherry SET ord = ?, flags = ? WHERE id = ?");
            for (int a = 0, N = dialogFilters.size(); a < N; a++) {
                MessagesController.DialogFilter filter = dialogFilters.get(a);
                state.requery();
